...
- **`Back Button`** *(button)* — Кнопка возврата к Шагу 1. Label: `← BACK TO OPERATOR DATA`. Очищает все поля Шага 2 при нажатии.

- **`Complete Button`** *(button)* — Кнопка завершения регистрации. Label: `COMPLETE REGISTRATION`. Состояние: неактивна (disabled), пока все три поля не заполнены валидными данными.
